...
ТИП ПОСТА:
КЕЙС = конкретная компания + конкретное действие (запуск/внедрение/партнёрство)
        + технологическая суть + измеримая ценность для отрасли
НОВОСТЬ = событие или факт без продуктовой конкретики:
  изменение ставок, кадровые перестановки, финрезультаты, общие обзоры
При сомнении — новость.
  ...
  "reason": "одно предложение почему"
    ...
    def check_relevance_and_classify(
        ...
        user = (
            ...
            f'"type": "case"/"news"/null, "case_count": 0}}'
        )

...
# ── Статические системные промпты (одинаковы для всех вызовов → лучше кэшируются) ──
...
типичные вводные фразы roundup-постов вроде "важные новости из мира
финансов и технологий, которые вы могли пропустить", "коротко о главном",
"что произошло за неделю", отсутствие единой компании-героя (или компания
указана как "мир"/"разное"/не названа).

Если пост упоминает 2+ разных компаний/проектов КАК РАВНОЗНАЧНЫЕ ПРИМЕРЫ
в рамках ОДНОГО ПОСТА — это ВСЕГДА digest, не case, даже если каждый
пример выглядит интересным И ДАЖЕ ЕСЛИ компании относятся к РАЗНЫМ
темам/отраслям (пост с новостями из крипты + ритейла + HR одним списком —
тоже digest, а не набор отдельных кейсов). Один case — это всегда один
герой с одним действием.

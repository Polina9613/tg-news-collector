...
def _get_pending_cases(offset: int = 0) -> tuple[dict | None, int]:
    """Возвращает (кейс_dict | None, всего_ожидающих)."""
    from db.base import get_session
    from db.models import NewsCard, TrendCase
    with get_session() as s:
        base = (
            s.query(TrendCase)
            .join(NewsCard, TrendCase.news_card_id == NewsCard.id)
            .filter(NewsCard.review_status.in_(["auto", "needs_review"]))
        )
        total = base.count()
        row = (
            s.query(TrendCase, NewsCard)
...
def _format_case_text(case: dict, current: int, total: int) -> str:
    url_line = (
        f"\n<a href='{case['source_url']}'>Источник</a>"
        if case.get("source_url") else ""
    )
    status_emoji = "⚠️" if case["review_status"] == "needs_review" else "🔵"
    return (
        f"{status_emoji} <b>Кейс на проверке ({current}/{total})</b>\n"
        f"ID: {case['id']}\n\n"
        f"<b>Тренд:</b> {case['trend_name'] or '—'}\n"
        ...
        f"{url_line}\n\n"
        f"Источник: {case['source_title'] or '—'} | Score: {case['relevance_score']}"
    )


def _review_keyboard(case_id: int, offset: int, total: int) -> InlineKeyboardMarkup:
    nav_row = []
    if offset > 0:
        nav_row.append(InlineKeyboardButton(text="⬅️", callback_data=f"rv:prev:{offset}"))
    if offset + 1 < total:
        nav_row.append(InlineKeyboardButton(text="➡️", callback_data=f"rv:next:{offset}"))

    rows = [
        [
            InlineKeyboardButton(text="✅ Одобрить",  callback_data=f"rv:approve:{case_id}:{offset}"),
            InlineKeyboardButton(text="❌ Отклонить", callback_data=f"rv:reject:{case_id}:{offset}"),
        ],
        [
            InlineKeyboardButton(text="✏️ Редактировать", callback_data=f"rv:edit:{case_id}"),
            InlineKeyboardButton(text="🗑 Удалить",        callback_data=f"rv:delete:{case_id}:{offset}"),
        ],
    ]
    if nav_row:
        rows.append(nav_row)
...
async def cmd_review(message: Message) -> None:
    case, total = _get_pending_cases(offset=0)
    if not case:
        await message.answer("Нет кейсов на проверке.")
        return
    await message.answer(
        _format_case_text(case, 1, total),
        reply_markup=_review_keyboard(case["id"], 0, total),
        link_preview_options=_NO_PREVIEW,
    )


# ── Callback-обработчики для /review ─────────────────────────────────────────

@router.callback_query(F.data.startswith("rv:"))
async def handle_review_callback(call: CallbackQuery, state: FSMContext) -> None:
    ...
    if action in ("approve", "reject", "delete"):
        ...
        if action == "delete":
            ...
            log_action(user["id"], f"{action}_case", "case", case_id, {"status": new_status})
            await call.answer("Одобрено" if action == "approve" else "Отклонено")
            new_offset = offset

        case, total = _get_pending_cases(offset=new_offset)
        if not case:
            await call.message.edit_text("Все кейсы проверены!")
        else:
            await call.message.edit_text(
                _format_case_text(case, new_offset + 1, total),
                reply_markup=_review_keyboard(case["id"], new_offset, total),
                link_preview_options=_NO_PREVIEW,
            )
        ...
        await call.message.answer(
            f"<b>Редактирование кейса #{case_id}</b>\n\nКакое поле изменить?",
